...
### TLV 编码消息
一些消息的格式是使用 TLV（类型，长度，值） 编码的，这意味着当消息达到时，消息的总长度被存储在
消息的开始。这一方式你可以立即知道对整个消息分配多大的内存。

TLV 编码使得内存的管理变得更加容易，你可以立刻知道对这个消息分配多大的内存。只有部分在结束时使用
缓冲区才会使得内存浪费。

TLV 编码的一个缺点是你要在消息的所有数据到达之前就分配好这个消息需要的所有内存，一些慢链接发送
一些大消息消耗掉你所有的内存，从而使你的服务器停止响应。

此问题的解决方法是使用包含多个 TLV 字段的消息格式，因此，服务器的内存被分配到每个字段，而不是
整个消息，并且是字段到达之后再分配内存。然而，一个大字段和大消息一样依然对你的内存管理有相同的
影响。

另一个解决方案就是对还未到达的消息设置超时时间，例如 10～15 秒。这个方案可使得你的服务在恰好有
很多大消息到来时可以恢复，但是，这仍使得你的服务器有一小段时间无法响应。另外，恶意的 DoS 攻击仍
然可以分配完你服务器的所有内存。

TLV 编码存在多个不同的变种，确切的说使用了多少字节，因此指定字段的类型和长度取决于每个单独的 TLV
编码。也有很多 TLV 编码将字段的长度放在首位，然后是字段类型，最后是字段值（一个 LTV 编码）。
虽然字段的顺序不同，但这仍是一个 TLV 的一种。

TLV 编码使得内存管理变得容易也是 HTTP 1.1 协议变得如此糟糕的原因之一。这也是他们试图在 HTTP 2.0
中修复的问题之一，其中数据在 TLV 编码的帧中传输。这也是我们为使用 TLV 编码的 VStack.co 项目
设计自己的网络协议的原因。
